...
struct obs_view {
	...
	/*
	 * obs中0通道的资源是transiton，而transiton内部包含的资源是scene。
	 * 1-6通道分配给了音频
	 */
...
struct obs_core_video {
	/*
	 * 适配的图形渲染系统，win使用d3d  mac使用opengl
	 * 图形的绘制渲染都是通过图形子系统完成。使用gpu
	 */
	graphics_t *graphics;
...
struct obs_context_data {
	...
	/**
	 * obs_source_info里create函数返回的对象指针
...
struct obs_source {
	/**
	 * 资源的上下文信息，包含设置参数、资源名称、信号、类型、私有标志等
	 */
	...
	/* 音频时间戳 对于复合类型资源(scene、group、transition) audio_ts 等于调用audio_render函数中计算的时间戳 */
	uint64_t audio_ts;
	...
	/* 保存的音频重采样器对应的原始音频格式，用来处理当原始音频格式发生变化的时候，重置重采样参数 */
	...
	/* 捕获音频数据回调函数和参数的数组 通过obs_source_add_audio_capture_callback接口注册 */
	DARRAY(struct audio_cb_info) audio_cb_list;

	/* 保存重采样、音频过滤(算法处理，比如降噪)之后的音频数据 */
	struct obs_audio_data audio_data;

	/* audio_data 中每一维数组的内存大小 */
	size_t audio_storage_size;

	/* 创建obs_source资源的时候设置默认值为 0xff */
	uint32_t audio_mixers;
	float user_volume;
	float volume;
	/* 音频的同步偏移量，可以加快或延迟音频的播放 */
	int64_t sync_offset;
...
struct obs_output {
	/* 上下文信息，包含了底层实际的对象指针 context->data */
	struct obs_context_data context;
...
struct obs_encoder {
	...
	uint32_t samplerate;
	/* 如果音频数据存储格式是planar，则planes=音频格式的通道数
	 * 如果音频存储格式是packet格式，那么planes=1 */
	size_t planes;
	size_t blocksize;
	size_t framesize;
	...
	/* 混流器index，如果是video  encoder则mixer_idx=0 */
	size_t mixer_idx;
	...
	/* 编码器参数是否有更新，如果为true，则在下次编码的时候需要先调用编码器的update */
