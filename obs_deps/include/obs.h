...
/* 获取延迟帧数，可能由于cpu或者系统消耗高导致一帧视频的处理时间超出了预设的时长 */
...
/*
 * 注册音频数据的回调函数接口，对应source的audio data将会传递到提供的callback函数
 * 此接口可以用来捕获特定source的音频数据
 */
...
/*
 * 设置资源的监听模式，如果值为 OBS_MONITORING_TYPE_MONITOR_AND_OUTPUT 则会通过扬声器输出音频
 */
 ...
 * 将采集到的音频数据通过处理(重采样、过滤处理)添加到音频数据的缓冲区 audio_input_buf
 * */
